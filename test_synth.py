...
import configparser
import ast
from VascGraph.GraphIO import ReadPajek, ReadNWKCS31
...
def preprocess_graph(g, scale=0.5):
    ...
    print('--Refining radius, this might take a while...')
...
if __name__=='__main__':
    ...
    '''
    compute diffusion spin echo response based on synth vascular graphs in 
    'data_synth'
    ...
    # read
    file=ReadNWKCS31(path_net=path+net_filename, 
                 path_attr=path+attr_filename)
    g=file.GetOutput()
    
    
    '''
    #or to read the pajek saved graph
    namepajek=''.join(attr_filename.split('.')[:-1])+'.pajek'
    g=ReadPajek(path+namepajek, mode='di').GetOutput()
    '''
    ...
    print('--Assign missing attributes...')
    og=vmri.Graphing.OxyGraph(g)
    og.AddType(art_vein=False, threshold=7.5)
